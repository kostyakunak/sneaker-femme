...
export default function SupplierNetwork() {
    ...
    const fetchStats = async () => {
        setLoading(true);
        try {
            console.log('[Frontend] Fetching sync stats...');
            const response = await axios.get('/api/admin/supplier-sync-stats');
            console.log('[Frontend] Stats received:', response.data);
            setStats(response.data.data);
        } catch (e) {
            console.error('[Frontend] Stats fetch failed:', e);
        } finally {
            setLoading(false);
        }
    };
    ...
    const runSync = async () => {
        ...
        setSyncing(true);
        try {
            console.log('[Frontend] Starting manual sync...');
            await axios.post('/api/admin/supplier-sync-run');
            console.log('[Frontend] Sync completed.');
            alert('Sync completed successfully.');
            await fetchStats();
        } catch (e) {
            console.error('[Frontend] Sync failed:', e);
